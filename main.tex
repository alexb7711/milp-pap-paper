...
the infrastructure that they rely upon. Much attention has been given to solving both problems simultaneously \citep{Wei2018,
...
optimal charging schedule. In \citep{Wei2018, Sebastiani2016, Wang2017}, initial interest is demonstrated in this topic.
...
considering different sources of battery degradation. \citep{Wang2017}, who also touches on the need for further research
...
the vehicles, and each vehicle visits the charger a single time \citep{Qarebagh2019}.
...
To determine the values for each of these decision variables, a MILP is formulated in \citep{Qarebagh2019} and shown here
...
\citep{Rodriguez2013}. A standard way of linearizing a bilinear term that contains a continuous and integer term is by
introducing a new term, an either/or constraint, and utilizing big-M notation \citep{Chen2010,Rodriguez2013}. Introducing
...
arrival to a maximum of the next arrival time (i.e. $\frac{a_i + a_{\gamma_i}}{2}$ to $a_{\gamma_i}$). The discharge is assumed to be linear and is calculated via $\lambda_i = \text{rand}(\frac{a_i + a_{\gamma_i}}{2},a_{\gamma_i})\zeta_{\gamma_i}$ where $1 \leq i \leq N$ and $\zeta_i$
