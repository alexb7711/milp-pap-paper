...
The PAP is a rectangle packing problem where a set of rectangles \(vmathbb{R}\) are attempted to be fit in a larger rectangle \(R\) as shown in Fig \cite{fig:packexample}. Both the set \(vmathbb{R\) and rectangle \(R\)'s width and height are used to represent quantifiable values. The rectangle packing problem is a subset of the packing problem and can be used to describe many real life problems \cite{}. One of these problems is the Berth Allocation Problem (BAP).

The BAP is a form of the rectangle packing problem where the set of rectangles describe incoming vessels to a be docked on a berth to be serviced as shown in Fig \cite{fig:bap} \cite{}. The BAP is commonly formed as a Mixed Integer Linear Program \cite{}. This formulation utilizes the BAP to help determine which queue the bus should be placed in to be charged.
