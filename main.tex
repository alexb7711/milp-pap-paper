...
%% \usepgfplotslibrary{external}                                                   % Pgfplots macros
%% \tikzexternalize[prefix=fig/]                                                   % Externalize the figures
...
\newcommand{\N}{266 }                                                           % Number of visits
...
\newcommand{\solvetime}{300 }                                                   % Solve time
...
(\fpeval{\bcharge * \batsize} KWh). Planning is done over a 24-hour time horizon. $n_C = \fpeval{\fast + \slow}$
chargers are utilized where \slow of the chargers are slow charging (\slows KWh) and \fast are fast charging (\fasts
KWh).
