...
The BAP has characteristics that are both convenient and undesirable properties; to explore these, consider the following MILP:
...
An undesirable property is that each vessel \(i\;; \forall 1 \leq i \leq N\) is assumed to be a new (i.e. unique) visit. This becomes a problem later when accounting for battery dynamics. \(S\) and \(T\) are allowed to be continuous or discrete. This allows the berth (queue) to either be left continuous, or discretized to accommodate \(Q\) chargers as shown in Fig \ref{fig:bap} \cite{Buhrkal2010}. Focus is now directed toward discussing the objective function and constraint properties.

The objective function \ref{eq:bapobjective} minimizes the time spent to service each vessel. Constraints \ref{subeq:baptime}-\ref{subeq:bapdelta} are used to prevent overlapping over both space and time as shown in Fig \ref{fig:nooverlap}.
