...
\begin{figure}
	...
	\begin{tikzpicture}[scale=0.35]
        ...
        \node[rectangle, draw, fill=gray, minimum width=2cm, minimum height = 1.2cm] at (12,2) {C};
...
\begin{figure}
	...
	\caption{Examples of different methods of overlapping. Space overlap: \(v_{k_1} < v_{i} + s[i] \therefore \delta_{k_{1}i} = 0\). Time overlap \(u_{k_1} < u_{j} + p[j] \therefore \sigma_{k_{2}j} = 0\). Both space and time overlap \(\sigma_{k_{3}i} = 0\) and \(\delta_{k_{3}j} = 0\).}
